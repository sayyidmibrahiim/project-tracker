...
| claude-mem   | Cross-session recall                                  | MANDATORY every session: recall at start (saves tokens); update on every important change (decisions, architecture, conventions, state) | Current code facts; verify source instead. Never dump everything |
