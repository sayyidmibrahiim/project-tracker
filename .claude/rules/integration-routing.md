...
| claude-mem   | Cross-session recall                                  | DISABLED (2026-07-09, perf: multi-second hook lag every tool call + 60s SessionStart timeouts). Use `_docs/session-notes.md` instead until re-enabled/fixed | Current code facts; verify source instead. Never dump everything |
| Headroom     | Optional local proxy                                  | Invoked explicitly via `claude-fast` (opt-in wrapper); keep only if `headroom doctor` shows real savings, not noise | Normal code lookup/search; never route by default (no fallback if proxy down) |
