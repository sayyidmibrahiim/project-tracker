...
Dashboard Master-Detail Redesign (2026-06-23): Redesigned the Dashboard screen from a 10-column table into a master-detail layout (38% master list, 62% detail panel) with full feature preservation, inline edits, and responsive layout. Updated design tokens in `styles.css` to DBS primary red (`#E60000`) and modern white canvas, and mapped compatibility aliases correctly. Updated `as-is-prototype-parity.test.mjs`, `dashboard-reopen.test.mjs`, and `components.test.mjs` for the new layout. Added new layout assertions in `dashboard-inline-edit.test.mjs`. Verified with Svelte check (PASS), full frontend tests (142/142 PASS), and Vite production build (PASS).

Claude Design System Migration (2026-06-22): Migrated the entire frontend styling to the warm-editorial Claude Design System defined in `DESIGN.md`. Updated global CSS custom properties in `styles.css` to define the warm cream canvas (`#faf9f5`), primary warm coral (`#cc785c`), ink text (`#141413`), soft cream card (`#efe9de`), and dark navy (`#181715`) color palette, alongside display serif typography stacks (`Tiempos Headline`, `Cormorant Garamond`) and humanist body sans (`StyreneB`, `Inter`). Ported these colors and borders across all Svelte components (`Dashboard.svelte`, `ProjectDetails.svelte`, `SubProjectTable.svelte`, `ConfirmModal.svelte`, `NewProjectForm.svelte`, `SecondBrain.svelte`, `Settings.svelte`, `FirstRunSetup.svelte`, `NotesEditor.svelte`, `FileActions.svelte`, `OutlookActions.svelte`, `ProjectActions.svelte`, `ProjectTransitions.svelte`, `RulesActions.svelte`, `SchedulerActions.svelte`, `TeamsActions.svelte`, `DashboardRowMenu.svelte`, `DisabledHint.svelte`, `EmailTemplateDialog.svelte`). Checked and verified with Svelte type check (`svelte-check` PASS), all 140 frontend unit/integration tests (`npm test` PASS), full Vite production build (`npm run build` PASS), and 1759 backend unit tests (`pytest` PASS). Visual rendering verified in the WebView preview environment.

AS_IS HTML prototype parity pivot (2026-06-18): user rejected current layout/design and set `redesign_ui/project_tracker_dbs_full_prototype_v2_AS_IS_PY_INJECTED.html` as the visual/layout contract. Production constraints remain: Svelte + TypeScript + Vite frontend, pywebview bridge, no raw static HTML runtime, no Vanilla JS state management. The Svelte shell now ports the prototype chrome/tokens/classes across `styles.css`, `App.svelte`, `Header.svelte`, `Sidebar.svelte`, Dashboard, Project Details, Second Brain, Report, Automations, Settings, and Outlook automation layout while preserving existing bridge-backed actions. Added `frontend/tests/as-is-prototype-parity.test.mjs` and updated stale layout assertions. Gates: `npm --prefix frontend run check` PASS (`110 FILES 0 ERRORS 0 WARNINGS`), `npm --prefix frontend test` PASS (`110 passed`), `npm --prefix frontend run build` PASS (Vite built to `web/static/`). Runtime/manual WebView2 verification still pending below.

AS_IS quick-win parity closure (2026-06-19): PRD remains feature/rule truth and the AS_IS HTML remains visual-only. Closed audited gaps without rebuilding working areas: Project Details CR dropdown now exposes REOPEN for POSTPONED/CANCELED and routes to `folder_reopen`; Report year filter now uses live `year_list` and filters debounce/apply directly; Settings theme is saved and Browse buttons call guarded native `util_choose_folder`; Second Brain adds real folder/text-file creation plus markdown toolbar insertion; Link Bank keeps the safe flat schema but adds backward-compatible rich fields (`details`, `tags`, `pinned`, `favorite`) plus category-first UI and category CRUD bridges; Automations shell now follows the AS_IS tab/chrome while preserving PRD Rules Engine access. Gates: `npm --prefix frontend run check` PASS (0 errors/warnings), `npm --prefix frontend test` PASS (120 passed), `npm --prefix frontend run build` PASS, touched backend pytest PASS (39 passed), `py_compile` PASS for touched Python. Windows manual WebView2/native folder picker/Outlook/Teams verification still pending.

Dashboard inline edit hardening slice (2026-06-21): `/impeccable harden Dashboard` narrowed to safer inline edit state. Replaced singleton `savingKey` with per-edit `savingKeys` plus `withSaving()` so concurrent CR link, datetime, Drone link/state, CR state, and reopen saves do not clear each other's disabled/spinner state. `withSaving()` reloads row data without clearing failed inline-save errors, so a rejected bridge update still shows the existing action banner after the row reverts. Added regression source guards in `frontend/tests/dashboard-inline-edit.test.mjs`; TDD RED first failed on the old singleton state / reload-clear behavior, then passed after implementation. Gates: targeted frontend test PASS (`126 passed` with name-pattern run), `npm --prefix frontend run check` PASS (`108 FILES 0 ERRORS 0 WARNINGS`), full `npm --prefix frontend test` PASS (`126 passed`), `npm --prefix frontend run build` PASS, `graphify update .` PASS (`25070 nodes`, `48998 edges`; HTML skipped over node limit). Browser preview skipped because the changed save-state/error-preservation behavior requires live pywebview bridge data; Vite preview would only show `BRIDGE_UNAVAILABLE`.

Dashboard professional polish slice (2026-06-22): `/impeccable polish Dashboard Menu Only` refined the live header/date-time badge, Dashboard controls, table sizing, row action trigger, CR/Drone saved-link display, CR State guard, dock symbols, and reduced-motion-safe transitions. Saved CR/Drone links now hide the paste input and show the extracted ID (fallback label) with copy/open controls; copy attempts rich `text/html` + plain URL fallback. Empty CR links now disable CR State as `Add CR Link First`. Gates: `npm --prefix D:/Ibrahim/Projects/project_tracker/frontend run check` PASS (`108 FILES 0 ERRORS 0 WARNINGS`), Dashboard-targeted frontend tests PASS (`42 passed`), `npm --prefix D:/Ibrahim/Projects/project_tracker/frontend run build` PASS, `graphify update .` PASS (`34321 nodes`, `66804 edges`). Fresh Vite preview PASS: no console logs, no server errors, snapshot showed modernized header date/time, Dashboard controls, table headers, and dock icons; inspectors confirmed `.date-time-badge`, `.dashboard-project-table`, and dock button sizing. Full `npm --prefix ... frontend test` was attempted but remains blocked by an unrelated pre-existing `Report Table` AS_IS parity assertion in `Report.svelte`; Dashboard-targeted coverage passed.

Dashboard polish slice (2026-06-22): `/impeccable polish Dashboard` applied the user-requested professional productivity pass. Fixed the Dashboard Svelte compile blocker caused by invalid `{#if ...}` fragments inside attribute/content positions. Removed the Dashboard metric command strip (`Total CR`, `Needs Review`, etc.), removed grouped table header chrome (`Project | Schedule | Drone | CR | Actions`), and removed the direct table outer box/shadow/padding so the table reads as a cleaner workspace surface. Replaced casual Dashboard/row-menu symbols (`＋`, `⚠`, `↗`, `⋮`, `🔒`) with text-first controls (`Add Project`, `Add Year`, `Open`, `More`, `Delete locked`) where this slice touched Dashboard surfaces; app-wide header/dock glyph vocabulary remains a separate visual pass if desired. Updated Dashboard source guards and AS-IS parity tests to reflect the approved user override away from the prior command-strip/group-header direction. Gates: `npm --prefix D:/Ibrahim/Projects/project_tracker/frontend run check` PASS (`108 FILES 0 ERRORS 0 WARNINGS`), Dashboard-targeted frontend tests PASS (`126 passed` with name-pattern run), `npm --prefix D:/Ibrahim/Projects/project_tracker/frontend run build` PASS, `/impeccable detect` PASS (`[]`) for `Dashboard.svelte` + `DashboardRowMenu.svelte`. Fresh Vite preview PASS: no console logs, no server errors, snapshot showed Dashboard status filters and compact table headers (`No`, `Project`, `Sub Project`, `Start`, `End`, `Drone Ticket`, `Drone State`, `CR Number`, `CR State`, `More`); inspector confirmed `.dashboard-table-scroll` has no box-shadow and white background.

Dashboard requested fixes slice (2026-06-20): user override applied over prior AS_IS/PRD conflicts. Dashboard project-name cell is centered; Dashboard Start/End are inline `datetime-local` editors backed by `project_update` and timezone-aware ISO conversion; Header removed unused `All CR` dropdown; left sidebar replaced by centered bottom dock with auto-hide hover zone and notification badge/popover flow. Drone rules updated: manual Drone `CANCELED` is rejected unless CR is already `CANCELED`; CR cancel cascades all Drone tickets to `CANCELED` while storing `previous_drone_state_before_canceled`; reopening from CANCELED restores stored Drone states; changing a Drone link resets that ticket to `UAT`. PRD updated for new Dashboard datetime edit, dock nav/notification UX, and Drone cancel/reopen/link-reset rules. Gates: targeted backend pytest PASS (`13 passed` with repo-local `--basetemp` after Windows temp symlink cleanup caused exit-1 despite passed tests), frontend check PASS (0 errors/warnings), full frontend tests PASS (`123 passed` before final hover fix; sidebar targeted test PASS after), frontend build PASS, `graphify update .` PASS. Live app launched with `D:/Ibrahim/Projects/project_tracker/.venv/Scripts/python.exe -m project_tracker.main`; no stdout/stderr output before manual stop. Code review agent attempts failed from model/auth config; manual review found and fixed the notification hover selector bug.

Dashboard layout redesign slice (2026-06-20): implemented approved `/impeccable shape Dashboard` direction for Dashboard-only layout. Added a command summary strip (`Total CR`, `Needs Review`, state counts) derived from existing Dashboard data with a frontend-only Needs Review filter for missing links/schedule data; kept status tabs and added reset affordance. Added grouped sticky table hierarchy (`Project`, `Schedule`, `Drone`, `CR`, `Actions`) while preserving PRD §11.15 columns, AS-IS dense layout, and solid red state selects. Softened internal table dividers and kept stronger group separators. Added frontend source guard coverage for command strip/grouped hierarchy. Gates: `npm --prefix frontend run check` PASS (0 errors/warnings), Dashboard-targeted frontend tests PASS (`124 passed`), full frontend tests PASS (`124 passed`), frontend build PASS. Browser preview verified command strip + group headers via snapshot/inspect/eval; screenshot capture timed out in preview tooling, so visual proof used DOM/a11y snapshot instead. Windows pywebview live-data visual check still pending.

Dashboard AS-IS parity + dummy/mock purge (2026-06-19): user rule restated — UI/layout/style taken from the AS_IS HTML prototype (`_reference/project_tracker_dbs_full_prototype_v2_AS_IS_PY_INJECTED.html`); app rules/user-flow/state guards taken from `PRD.md`. Dashboard now follows AS-IS: solid DBS-red active status tabs, `▦ CR - Project Summary Table` card, dark `#111` uppercase 9px header row, state selects restored to AS-IS solid red `state-combo` styling (the prior Notion soft-tag chip colour overrides removed). **Superseded on 2026-06-20:** user requested Dashboard Start/End inline datetime editing, so PRD §11.15 and Dashboard now allow `datetime-local` edits backed by `project_update`. Dummy/mock leftovers purged: deleted `frontend/src/lib/devMockBridge.ts` (DEV-only sample `SOP CR`/`project_tracker` data), removed the `?preview` mock-bridge branch from `frontend/src/main.ts`, deleted the unused `frontend/src/lib/components/PagePlaceholder.svelte`, and removed its import/`pageShells`/fallback from `frontend/src/App.svelte`. Removed dead inline `toDatetimeLocal`/`fromDatetimeLocal`/`saveProjectDatetime`/`pad2` plus orphaned `.dash-date-input` styles from `Dashboard.svelte`. "2 dummy list project" root cause found: production `create_js_api()` defaulted `db_path` to a shared `%TEMP%/project_tracker_cache.db` that pytest also writes to, so test fixtures leaked `renamed-project`/`r4-proj` rows into the live Dashboard cache. Fix: `app_web.run()` now passes `db_path=app_config_dir()/project_tracker_cache.db` so the desktop app owns its own rebuildable cache, isolated from the test temp cache; tests still use the temp default so nothing else changed. Regression guard added: `test_run_uses_app_config_cache_path`. Gates: `npm --prefix frontend run check` PASS (0/0), `npm --prefix frontend test` PASS (120 passed), `npm --prefix frontend run build` PASS, `py_compile` PASS, backend pytest PASS (`1751 passed, 20 skipped` — 1 pre-existing unrelated `test_link_bank_store_write_read_round_trip_current_schema` failure confirmed failing on `main`/stash too, not caused by this slice). App launched live (`python -m project_tracker.main`) with no stderr/stdout errors. Windows manual WebView2 visual confirmation of the AS-IS Dashboard still pending (user eyes).

pywebview folder dialog deprecation fix (2026-06-19): `util_choose_folder` now prefers `webview.FileDialog.FOLDER` and falls back to legacy `webview.FOLDER_DIALOG` only for older pywebview compatibility, removing the runtime warning `[pywebview] FOLDER_DIALOG is deprecated...` on current pywebview. Test fixture updated to assert the new enum path plus legacy fallback. Verification pending: Cowork shell remained in startup state during repeated targeted attempts, so run `python -m pytest tests/test_phase_c_js_api_util_choose_folder.py -v` and `python -m py_compile project_tracker/web/js_api.py tests/test_phase_c_js_api_util_choose_folder.py` locally if needed.
...
- **CR POSTPONED/CANCELED → Drone mirror fix** (2026-06-20): user clarified PRD behavior. CR→CANCELED now auto-sets all Drone tickets to CANCELED; CR→POSTPONED now auto-sets all Drone tickets to POSTPONED. Both preserve each ticket's prior Drone state and `folder_reopen` restores that prior state when returning to UAT_PREPARE. Updated PRD §9.3 to include auto-only Drone POSTPONED. Relevant pytest: `55 passed`.
