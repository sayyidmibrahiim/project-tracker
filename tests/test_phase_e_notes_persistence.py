...
def test_rte_pipeline_default_margins_are_narrow():
    """Page defaults are Word 'Narrow' (12.7mm) — printable width 184.6mm."""
    from infrastructure.docx_writer import DEFAULT_DOCUMENT_SETTINGS, content_width_mm

    assert DEFAULT_DOCUMENT_SETTINGS["margin_left_mm"] == 12.7
    assert DEFAULT_DOCUMENT_SETTINGS["margin_right_mm"] == 12.7
    assert abs(content_width_mm(DEFAULT_DOCUMENT_SETTINGS) - 184.6) < 0.01


def test_rte_pipeline_export_clamps_oversized_image_and_table(temp_project):
    """Images/tables wider than the printable area are clamped, never cut off."""
    import base64 as b64
    ...
    cell = lambda text, px: {  # noqa: E731 - local table-cell builder
        "type": "tableCell",
        "attrs": {"colwidth": [px]},
                ...
                "content": [
                    {
                        ...
                        "attrs": {
                            ...
                            "width": 5000,  # far beyond the printable width
                        },
                    }
                ],
            },
            {"type": "table", "content": [{"type": "tableRow", "content": [cell("a", 900), cell("b", 900)]}]},
        ],
    }
    ...
    width_mm = content_width_mm(DEFAULT_DOCUMENT_SETTINGS)
    doc = Document(str(docx))
    max_emu = int(Mm(width_mm))
    assert doc.inline_shapes[0].width <= max_emu + 1000  # EMU rounding headroom

    max_dxa = width_mm / 25.4 * 96.0 * 15  # mm -> px@96dpi -> dxa
    row = doc.tables[0].rows[0]
    total_dxa = 0
    for table_cell in row.cells:
        tc_w = table_cell._tc.get_or_add_tcPr().find(qn("w:tcW"))
        assert tc_w is not None
        total_dxa += int(tc_w.get(qn("w:w")))
    assert total_dxa <= max_dxa + 15  # rounding headroom (1 dxa per column)


def test_rte_pipeline_sidecar_dir_is_hidden_on_windows(rte_docx):
    import sys

    svc, docx = rte_docx["service"], rte_docx["docx"]
    svc.save_document(docx, {"content": _doc("x"), "base_revision": 0})
    ...
    if sys.platform == "win32":
        ...
        assert attrs != -1 and attrs & 0x02  # FILE_ATTRIBUTE_HIDDEN


def test_rte_pipeline_save_refreshes_document_settings(rte_docx):
    """Sidecars written under the old 20mm defaults pick up Narrow on save."""
    import json as jsonlib

    svc, docx = rte_docx["service"], rte_docx["docx"]
    svc.save_document(docx, {"content": _doc("a"), "base_revision": 0})
    src_path = svc.source_path(docx)
    source = jsonlib.loads(src_path.read_text(encoding="utf-8"))
    source["document_settings"]["margin_left_mm"] = 20
    src_path.write_text(jsonlib.dumps(source), encoding="utf-8")

    svc.save_document(docx, {"content": _doc("b"), "base_revision": 1})
    updated = jsonlib.loads(src_path.read_text(encoding="utf-8"))
    assert updated["document_settings"]["margin_left_mm"] == 12.7
